...
public class DFSMazeSolver {
    ...
    public List<Coordinate> solve(Maze maze) {
        ...
        if (explore(maze, maze.getInitialKivaLocation()
            .getX(),
            maze.getInitialKivaLocation()
                .getY(),
            path)) {
            return path;
        }
    ...
    private boolean explore(Maze maze, int row, int col, List<Coordinate> path) {
        if (!maze.isValidLocation(row, col) || maze.isObstacle(row, col) || maze.isExplored(row, col)) {
            return false;
        }
